...
{ lib, pkgs }:
  let
    ...
    mkInstallPhase = args: ''
        ...
        fi" > $out/bin/make-flake
        chmod +x $out/bin/make-flake
    '';
  in
    with pkgs;
    stdenv.mkDerivation {
      pname = lib.strings.sanitizeDerivationName "flake-make";
      version = "0.1.0";
